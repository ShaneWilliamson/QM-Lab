...
DISPLAY=:1 xvfb-run java -jar ~/selenium/selenium-server-standalone-2.52.0.jar -role hub &
java -Dwebdriver.chrome.driver=src/Maven/src/test/resources/selenium_standalone_binaries/linux/googlechrome/64bit/chromedriver -jar selenium-server-standalone-2.53.0.jar -role node -hub http://localhost:4444/grid/register -port 5555 &
