//////////////////////////
// qm_lab_joint_classes //
//////////////////////////
/**
* Different classes created for QM lab to be used by joint.js
...
joint.shapes.basic.Rect.prototype.setSize = function(width, height) {
	...
	if (this.attributes.type === "QMLab.Agent" || this.attributes.type === "QMLab.ImageNode" || 
		...
		this.attributes.type === "QMLab.Intervention" || this.attributes.type === "QMLab.FinalState")
	{
 ...
 * @returns {colour} colour of the node
 * @memberOf qm_lab_joint_classes
 */
 ...
 *   parameters are passed, it will set the appropriate field in the link.
 *
 ...
 *
 * "rouned" - like the above, this sets the lines to straight. However, this
 *   causes the angles to "round" themselves
 *
 ...
 * @preconditions "pos" is a valid point with an "x" and "y" field
     If any other parameters are passed, it will set the appropriate field in the link
		...
		connector : if passed, must be one of three valid connector types:
				    ...
				    "smooth" - This causes the link to have no straight edges, and instead
					           gradually curve its way between vertices
 * @postconditions post: Returns a local link with source at the position "pos"
 ...
 * @preconditions The localLink exists
 * @postconditions No change
 * @returns {localLink} The source of the localLink
 * @memberOf qm_lab_joint_classes
 */
 ...
 * @postconditions The source of the node has been set to the passed in source.
 * @memberOf qm_lab_joint_classes
 */
 ...
 *   graph, source be a point with valid "x" and "y" fields.
 * @param {coordinate} source the starting point with 'x' and 'y' fields 
 * @preconditions The localLink exists
 ...
 *   "id". Otherwise, it will return a point that has fields "x" and "y"
 *
...
/**
 * Setter for localLink end node. Since this sets by using the cell's "id", the
 ...
 *   graph, source be a point with valid "x" and "y" fields.
 * @param {coordinate} target a valid point with "x" and "y" fields
 * @preconditions The localLink exists
 * @postconditions The source of the node has been set to the passed in source
 * @memberOf qm_lab_joint_classes
 */
...
joint.dia.Link.prototype.setPositiveTransition = function() {
	this.attr({
		'.marker-target': { d: 'M 10 0 L 0 5 L 10 10 z M 0 20 L 0 30 M -5 25 L 5 25 M 0 -20' },
	});
};
...
/*
This is the shape definition of the "Final State" type. It extends joint.js' Circle shape,
allowing us to easily customize it.
*/
/**
 * 
 ...
 * @memberOf qm_lab_joint_classes
 */
joint.shapes.QMLab.FinalState = joint.shapes.basic.Circle.extend({

	markup: '<g class="rotatable"><g class="scalable"><circle/><path/></g><text/></g>',

	defaults: joint.util.deepSupplement({

		type: 'QMLab.FinalState',
