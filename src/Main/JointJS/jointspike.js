	...
	function onstartRun(){
		...
		document.getElementById("debugButton").addEventListener('mousedown', function () {
			var cells = graph.getCells();
			console.log(cells);
			for (i = 0; i < cells.length; i++) {
				console.log(rootModel.getRoot().get(cells[i].id));
			}
		});
	...
	//Ensure the graph is never out of date
	...
	function paperOnMouseUp(e) {
		...
		if (globalState == "ADDNODE") {
			createNode(curMousePos);
	...
	joint.dia.Link.prototype.setColour = function(colour) {
		this.attr({'.connection': { stroke: colour}});
		console.log(this);
	}
